  ...
  try {
    ...
    const defaultConfig = {
      ...
      aiPricing: {
        ...
        // Voice (STT: $ per second, TTS: $ per character)
    ...
    // Merge existing settings with default configuration
    const userSettings = (systemTenant.settings as any) || {};
    
    // Ensure all pricing entries are converted to the new { inputPrice, outputPrice } format
    ...
    if (userSettings.aiPricing) {
      Object.keys(userSettings.aiPricing).forEach((modelId) => {
        ...
        if (rate) {
          // If it has the old structure, map it:
          // retailInput/wholesaleInput -> inputPrice
          // retailOutput/wholesaleOutput -> outputPrice
