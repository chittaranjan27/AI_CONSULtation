...
${stepInputType === "text"
          ...
          : `4. SPOKEN OPTIONS (MANDATORY): For every response/query in this flow, you MUST call the 'show_options' tool to display interactive buttons on the screen, AND list them naturally in your speech so they are spoken. If the step does not define explicit options, generate logical options (e.g. yes/no, range values, or common answers) so the user can navigate by clicking.`
        }
5. PRODUCT RECOMMENDATIONS: When recommending products, mention their names and key benefits in your speech, and ALSO call the 'fetch_products' tool to show the product cards. All prices and transactions must be discussed and represented in Dirham (د.إ) only (e.g. '150 د.إ'). Never use the dollar ($) symbol or mention USD.
6. RESPONSIVENESS: Keep responses natural and moderate in length (3-4 sentences max per turn). Avoid long paragraphs but provide enough detail to answer the user's questions naturally.
